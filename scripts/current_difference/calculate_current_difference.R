...
embed_vector <- function(vec, pascal){
  ...
  if(length(vec) > 5000) vec <- vec[1:5000]
...
x <- 1
pascal <- list()
for (i in 1:10000) {x <- c(0, x) + c(x, 0); pascal[[i]] <- x }
Sys.time()
signal_mappings_nat_unnested2 <- signal_mappings_nat_unnested[
  n_dac >= 5, .(dac_embed = as.numeric(embed_vector(dac, pascal))), by = .(read_id, contig_id, direction,  type)
]
Sys.time()
